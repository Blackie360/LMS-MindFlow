...
export function AuthForm({ mode = "signin" }: AuthFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    console.log("Form submission:", { formMode, email, name })
    ...
    try {
      if (formMode === "signup") {
        ...
        if (error) {
          ...
          if (error.code === "USER_ALREADY_EXISTS" || /unique|exists|duplicate/i.test(error.message || "")) {
            errorMessage = "An account with this email may already exist. Try signing in or resetting your password."
          } else if (error.message?.includes("password")) {
            errorMessage = "Password must be at least 8 characters long."
          } else if (error.message?.includes("network") || error.message?.includes("fetch")) {
            errorMessage = "Network error. Please check your connection and try again."
          } else if (/verify|verification|email.*sent/i.test(error.message || "")) {
            errorMessage = "We sent a verification email. Please verify your email, then sign in."
